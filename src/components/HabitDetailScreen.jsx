...
export default function HabitDetailScreen({ habit, isCompletedToday, onToggle, onDelete, onRename, onBack, theme }) {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: theme.bg }}>
          ...
          <div className="flex-1 min-w-0">
            {editingName ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => { setNameInput(habit.name); setEditingName(true); }}
      ...
      <div className="flex-1 px-5 pb-8 overflow-y-auto">
        ...
        <button
          onClick={onToggle}
          className="w-full py-3.5 rounded-2xl font-bold text-sm mb-5 transition-all active:scale-[0.97]"
        ...
        >
          ...
          <div className="flex items-center justify-between mb-4" dir="ltr">
            <button
              onClick={prevMonth}
              className="w-8 h-8 rounded-xl flex items-center justify-center text-gray-500 hover:text-gray-900 transition-colors text-xl"
              style={{ background: 'rgba(0,0,0,0.06)' }}
            >‹</button>
            <span className="font-semibold text-sm" style={{ color: theme.t1 }} dir="rtl">
              {formatMonthYear(year, month)}
            </span>
            <button
              onClick={nextMonth}
              disabled={isCurrentMonth}
              className="w-8 h-8 rounded-xl flex items-center justify-center text-gray-500 hover:text-gray-900 transition-colors text-xl disabled:opacity-25"
              style={{ background: 'rgba(0,0,0,0.06)' }}
            >›</button>
          </div>
          ...
          >
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-3 rounded-xl text-gray-600 text-sm font-medium active:scale-95"
